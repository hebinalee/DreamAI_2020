...
class Image_Audio_Ensemble(nn.Module):
    def __init__(self, classes_num, mode = '', extractor=True):
        
        super (Image_Audio_Ensemble, self).__init__()
        
        self.classes_num = classes_num
        self.extractor = extractor
        
        if self.extractor == True:
            self.MFCC_model = ResNet22(classes_num, audio_mode='mfcc', extractor=self.extractor)
            self.Image_model = models.resnet50(pretrained=True)
            num_ftrs = self.Image_model.fc.in_features
            self.Image_model.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            self.Image_model.fc = nn.Linear(num_ftrs, 2048)
            
            self.ext_fc = nn.Linear(2048*2, 1024)
            self.last_fc = nn.Linear(1024, classes_num)
        
    def forward(self, image, audio):
        
        if self.extractor == True:
            mfcc_feature = self.MFCC_model(audio)
            Image_feature = self.Image_model(image)
            
            fusion_feature = torch.cat((mfcc_feature, Image_feature), dim=1)
            
            out = self.ext_fc(fusion_feature)
            out = F.dropout(out, p=0.2, inplace=True)
            out = self.last_fc(out)
        
        return out

class MFCC_MEL_Ensemble(nn.Module):
    def __init__(self, classes_num, mode ='', extractor=True):
        
        super (MFCC_MEL_Ensemble, self).__init__()
        
        self.classes_num = classes_num
        self.extractor = extractor
        
        if self.extractor == True:
            self.MFCC_model = ResNet22(classes_num, audio_mode='mfcc', extractor=True)
            self.MEL_model = ResNet22(classes_num, audio_mode='mel', extractor=True)
            
            self.fc_audioset = nn.Linear(2048, classes_num, bias=True)
            init_layer(self.fc_audioset)
        else :
            self.MFCC_model = ResNet22(classes_num, audio_mode='mfcc', extractor=False)
            self.MEL_model = ResNet22(classes_num, audio_mode='mel', extractor=False)
        
    def forward(self, x):
        
        if self.extractor == True:
            mfcc_feature = self.MFCC_model(x)
            mel_model = self.MEL_model(x)
            
            x = mfcc_feature + mel_model
            x /= self.classes_num
            
            output = self.fc_audioset(x) 

        else :
            mfcc_out = self.MFCC_model(x)
            mel_out = self.MEL_model(x)
        
            output = mfcc_out + mel_out
            output /= self.classes_num
        
        return output


class ResNet22(nn.Module):
    def __init__(self, classes_num, audio_mode='', extractor=True):
...
#         # Spec augmenter
        ...
        if self.extractor == False :
            self.fc_audioset = nn.Linear(2048, classes_num, bias=True)
            self.init_weights()

    def init_weights(self):
        init_bn(self.bn0)
        if self.extractor == False : init_layer(self.fc_audioset)
    ...
    def forward(self, x, mode='train', mixup_lambda=None):
        ...
        if self.audio_mode =='mfcc': x = self.mfcc(x)
        elif self.audio_mode == 'mel': x = self.mel_spectrogram(x)
        x = x 
        ...
        if self.extractor == False : output = self.fc_audioset(x) 
        else : output = x
...
#         del state_dict['fc1.bias']
        #checkpoint = torch.load('./ResNet54_mAP=0.429.pth')
